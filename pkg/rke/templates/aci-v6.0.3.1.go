...
{{- end}}
      ...
      initContainers:
        - name: cnideploy
{{- if ne .UseDigest "false"}}
          image: {{.AciCniDeployContainerSha}}@sha256:{{.CnideployVersion}}
{{- else}}          
...
{{- end}}
      containers:
        - name: aci-containers-host
{{- if ne .UseDigest "false"}}
          image: {{.AciHostContainerSha}}@sha256:{{.AciContainersHostVersion}}
{{- else}}          
...
{{- end}}
{{- if ne .UseDigest "false"}}
          image: {{.AciOpflexContainerSha}}@sha256:{{.OpflexAgentVersion}}
{{- else}}          
...
{{- if eq .RunOpflexServerContainer "true"}}
        - name: opflex-server
{{- if ne .UseDigest "false"}}
          image: {{.AciOpflexServerContainerSha}}@sha256:{{.OpflexAgentVersion}}
{{- else}}          
...
{{- if ne .OpflexMode "overlay"}}
        - name: mcast-daemon
{{- if ne .UseDigest "false"}}
          image: {{.AciMcastContainerSha}}@sha256:{{.OpflexAgentVersion}}
{{- else}}          
...
{{- end}}
      containers:
        - name: aci-containers-openvswitch
{{- if ne .UseDigest "false"}}
          image: {{.AciOpenvSwitchContainerSha}}@sha256:{{.OpenvswitchVersion}}
{{- else}}          
...
{{- end}}
        - name: aci-containers-controller
{{- if ne .UseDigest "false"}}
          image: {{.AciControllerContainerSha}}@sha256:{{.AciContainersControllerVersion}}
{{- else}}          
